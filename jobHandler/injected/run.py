...
try:
    ...
    async def handle_register(websocket, message):
        ...
        if 'client' in message:
            ...
            if 'worker' in message:
                asp_temp_var_cc09d372046111f0b89d9f0ee9e28e0e = message['type']
                aspAdliVarLog(asp_temp_var_cc09d372046111f0b89d9f0ee9e28e0e, 10)
                logger.info(21)
                workers[asp_temp_var_cc09d372046111f0b89d9f0ee9e28e0e] = websocket
                aspAdliVarLog(workers[asp_temp_var_cc09d372046111f0b89d9f0ee9e28e0e], 11)
                logger.info(22)
                await send_to_worker(workers[message['type']], message)
    ...
    async def handle_request(websocket, message):
        ...
        if message['type'] in workers:
            logger.info(26)
            await workers[asp_temp_var_cc09d373046111f0b89d9f0ee9e28e0e].send(json.dumps(message))
            aspAdliVarLog(workers[asp_temp_var_cc09d373046111f0b89d9f0ee9e28e0e], 15)
        else:
            logger.info(27)
            message['response'] = f"message['type'] worker isn't initialized"
    ...
    async def handle_message(websocket, message):
        logger.info(38)
        aspAdliVarLog(websocket, 22)
        aspAdliVarLog(message, 23)
        logger.info(39)
        print(f'\nReceived message: {message}')
        logger.info(40)
        if message['code'] == MSG_TYPE['REGISTER']:
    ...
    async def receieve_message(websocket):
        ...
        async for message in websocket:
            ...
            logger.info(49)
            message = json.loads(message)
            aspAdliVarLog(message, 24)
            logger.info(50)
            asp_uid = message['asp_uid']
            aspAdliVarLog(asp_uid, 25)
            logger.info(51)
            await handle_message(websocket=websocket, message=message)
            logger.info(48)
